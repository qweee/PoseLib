...
std::vector<double> *output_focals, std::vector<double> *output_ks) {
	...
	for (int i = 0; i < 4; ++i) {
		...
		if (std::abs(x[i](0)) < SMALL_NUMBER) {
			...
			B(i, 7) = X[i](0) * N(1, 3) + X[i](1) * N(3, 3) + X[i](2) * N(5, 3) + N(7, 3); // 1			
		} else {
			A(i, 0) = x[i](0) * X[i](0);
			A(i, 1) = x[i](0) * X[i](1);
			A(i, 2) = x[i](0) * X[i](2);
			...
			B(i, 7) = X[i](0) * N(0, 3) + X[i](1) * N(2, 3) + X[i](2) * N(4, 3) + N(6, 3); // 1			
		}
	...
	for (int i = 0; i < 12; ++i) {
		...
		poses->push_back(p);
		output_focals->push_back(focal);
		output_ks->push_back(lambda);
